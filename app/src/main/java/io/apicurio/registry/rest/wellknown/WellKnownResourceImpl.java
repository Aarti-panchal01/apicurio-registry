    ...
    public AgentSearchResults searchAgentsAdvanced(AgentSearchRequest request) {
        ...
        if (f != null) {
            ...
            if (f.getCapabilities() != null) {
                for (Map.Entry<String, Object> entry : f.getCapabilities()
            ...
            if (f.getLabels() != null) {
                for (Map.Entry<String, Object> entry : f.getLabels()
                        .getAdditionalProperties().entrySet()) {
                    filters.add(SearchFilter.ofLabel(entry.getKey(),
        ...
        int safeLimit = Math.max(1, Math.min(
                request.getLimit() != null ? request.getLimit() : 20, 500));
